...
describe("MyERC20Contract", function(){
    ...
    beforeEach(async function() {
        ...
        console.log(someAddress.address);
        someOtherAddress = (await ethers.getSigners())[2];
        console.log(someOtherAddress.address);
    });
    ...
    describe("when I have 10 tokens", function(){
     ...
     describe("when I transfer 15 tokens", function(){
        it("should revert the transaction", async function () {
            await expect (myERC20Contract.connect(someAddress).transfer(someOtherAddress.address,15)).to.be.revertedWith("ERC20: transfer amount exceeds balance");
